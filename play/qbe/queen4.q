...
@true.10
    %.24 =w call extern $printf(l $.s1)
    jmp @if.end.12
@false.11
    %.26 =w call extern $printf(l $.s2)
@if.end.12
    %.29 =w loadw %_i
    %.28 =w add %.29, 1
    storew %.28, %_i
    jmp @while.cond.6
@while.end.8
    %.30 =w call extern $printf(l $.s3)
    %.33 =w loadw %_j
    %.32 =w add %.33, 1
    storew %.32, %_j
    jmp @while.cond.3
@while.end.5
    %.34 =w call extern $printf(l $.s4)
    ret
}
...
@if.32
    ...
    %.8 =w loadw %_Q
    %.6 =w ceqw %.7, %.8
    jnz %.6, @true.33, @false.34
@true.33
    %.10 =l loadl %_board
    %.11 =w loadw %_Q
    call $print(l %.10, w %.11)
    ret 1
@false.34
    storew 0, %_i
@while.cond.35
    ...
    %.17 =w loadw %_Q
    %.15 =w csltw %.16, %.17
    jnz %.15, @while.body.36, @while.end.37
@while.body.36
@if.38
    %.20 =w loadw %_i
    %.21 =w loadw %_j
    %.22 =l loadl %_board
    %.23 =w loadw %_Q
    %.19 =w call $chk(w %.20, w %.21, l %.22, w %.23)
    %.18 =w ceqw %.19, 0
    jnz %.18, @true.39, @false.40
@true.39
    %.29 =l loadl %_board
    %.30 =w loadw %_i
    %.31 =l extsw %.30
    ...
    %.25 =w add %.36, 1
    storew %.25, %.26
    %.39 =w loadw %_nSolutions
    %.42 =w loadw %_j
    %.41 =w add %.42, 1
    %.44 =l loadl %_board
    %.45 =w loadw %_Q
    %.40 =w call $go(w %.41, l %.44, w %.45)
    %.38 =w add %.39, %.40
    storew %.38, %_nSolutions
    %.50 =l loadl %_board
    %.51 =w loadw %_i
    %.52 =l extsw %.51
    %.53 =l mul 8, %.52
