...
: TYPEDEF                                               { PP(pt, "TYPEDEF  =>  storage_class_specifier"); $$ = mktype(pt_storage_class_specifier, (btypeid_t) T_TYPEDEF, $%); }
| EXTERN                                                { PP(pt, "EXTERN  =>  storage_class_specifier"); $$ = mktype(pt_storage_class_specifier, (btypeid_t) T_EXTERN, $%); }
| STATIC                                                { PP(pt, "STATIC  =>  storage_class_specifier"); $$ = mktype(pt_storage_class_specifier, (btypeid_t) T_STATIC, $%); }
| AUTO                                                  { PP(pt, "AUTO  =>  storage_class_specifier"); $$ = mktype(pt_storage_class_specifier, (btypeid_t) T_AUTO, $%); }
| REGISTER                                              { PP(pt, "REGISTER  =>  storage_class_specifier"); $$ = mktype(pt_storage_class_specifier, (btypeid_t) T_REGISTER, $%); }
;

type_specifier
: VOID                                                  { $$ = mktype(pt_type_specifier, (btypeid_t) T_VOID, $%); }
| CHAR                                                  { $$ = mktype(pt_type_specifier, (btypeid_t) T_CHAR, $%); }
| SHORT                                                 { $$ = mktype(pt_type_specifier, (btypeid_t) T_SHORT, $%); }
| INT                                                   { $$ = mktype(pt_type_specifier, (btypeid_t) T_INT, $%); }
| LONG                                                  { $$ = mktype(pt_type_specifier, (btypeid_t) T_LONG, $%); }
| FLOAT                                                 { $$ = mktype(pt_type_specifier, (btypeid_t) T_FLOAT, $%); }
| DOUBLE                                                { $$ = mktype(pt_type_specifier, (btypeid_t) T_DOUBLE, $%); }
| SIGNED                                                { $$ = mktype(pt_type_specifier, (btypeid_t) T_SIGNED, $%); }
| UNSIGNED                                              { $$ = mktype(pt_type_specifier, (btypeid_t) T_UNSIGNED, $%); }
| BOOL                                                  { $$ = mktype(pt_type_specifier, (btypeid_t) T_BOOL, $%); }
| COMPLEX                                               { $$ = mktype(pt_type_specifier, (btypeid_t) T_COMPLEX, $%); }
| IMAGINARY                                             { $$ = mktype(pt_type_specifier, (btypeid_t) T_IMAGINARY, $%); }
...
type_qualifier
: CONST                                                 { $$ = mktype(pt_type_qualifier, (btypeid_t) T_CONST, $%); }
| RESTRICT                                              { $$ = mktype(pt_type_qualifier, (btypeid_t) T_RESTRICT, $%); }
| VOLATILE                                              { $$ = mktype(pt_type_qualifier, (btypeid_t) T_VOLATILE, $%); }
;
...
| '*' type_qualifier_list                               { PP(pt, "'*' type_qualifier_list   =>   pointer"); $$ = node(pt_pointer, mktypequalifierlist($2, 0, $%), 0, $%); }
| '*' pointer                                           { PP(pt, "'*' pointer   =>   pointer"); $$ = node(pt_pointer, mktype(T_PTR, 0, $%), $2, $%); }
| '*' type_qualifier_list pointer                       { PP(pt, "'*' type_qualifier_list pointer   =>   pointer"); $$ = node(pt_pointer, mktypequalifierlist($2, 0, $%), $3, $%); }
;
